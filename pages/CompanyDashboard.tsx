
import React from 'react';
import { styles, primaryBlue, primaryPink, borderLight, textPrimary, textSecondary } from '../styles';
import { dummyUsers, initialConversations } from '../data';

type CompanyDashboardProps = {
    onLogout: () => void;
    onOpenChat: (conv: any) => void;
    onViewStudent: (id: string) => void;
};

export const CompanyDashboard: React.FC<CompanyDashboardProps> = ({ onLogout, onOpenChat, onViewStudent }) => {
    // 簡易的な企業用データ
    const stats = [
        { label: '閲覧数', value: '1,280', color: primaryBlue },
        { label: 'お気に入り', value: '45', color: primaryPink },
        { label: 'メッセージ', value: '12', color: '#10B981' }
    ];

    const recentInterests = [
        { id: 's1', name: '山本 恒一', univ: '香川大学', time: '10分前' },
        { id: 's2', name: '佐藤 未来', univ: '高松大学', time: '1時間前' },
        { id: 's5', name: '中村 陽斗', univ: '徳島大学', time: '3時間前' },
    ];

    return (
        <div style={{ ...styles.appContainer, backgroundColor: '#F8FAFC' }}>
            <header style={{
                padding: '1.25rem 1.5rem', backgroundColor: '#FFFFFF', borderBottom: `1px solid ${borderLight}`,
                display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100
            }}>
                <div>
                    <h2 style={{ fontSize: '1.25rem', fontWeight: '900', margin: 0 }}>株式会社タダノ</h2>
                    <p style={{ fontSize: '0.75rem', color: textSecondary, margin: 0 }}>採用担当ダッシュボード</p>
                </div>
                <button onClick={onLogout} style={{ ...styles.button, backgroundColor: '#F1F5F9', color: textSecondary, padding: '0.5rem 1rem' }}>ログアウト</button>
            </header>

            <main style={{ padding: '1.5rem 1.5rem 120px' }}>
                {/* Stats */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
                    {stats.map(s => (
                        <div key={s.label} style={{ backgroundColor: '#FFFFFF', padding: '1.25rem', borderRadius: '24px', textAlign: 'center', border: `1px solid ${borderLight}` }}>
                            <div style={{ fontSize: '1.5rem', fontWeight: '900', color: s.color, marginBottom: '4px' }}>{s.value}</div>
                            <div style={{ fontSize: '0.7rem', fontWeight: '800', color: textSecondary }}>{s.label}</div>
                        </div>
                    ))}
                </div>

                {/* Main Action */}
                <button style={{
                    width: '100%', padding: '1.25rem', borderRadius: '24px', border: 'none',
                    background: `linear-gradient(135deg, ${primaryBlue}, #1d4ed8)`, color: '#fff',
                    fontWeight: '900', fontSize: '1.1rem', marginBottom: '2.5rem',
                    boxShadow: `0 10px 25px ${primaryBlue}33`, cursor: 'pointer'
                }}>
                    インターン情報を編集・投稿
                </button>

                {/* Section: Recent Interests */}
                <section style={{ marginBottom: '2.5rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                        <h3 style={{ fontSize: '1.1rem', fontWeight: '900', margin: 0 }}>あなたの企業に興味を持った学生</h3>
                        <span style={{ fontSize: '0.8rem', color: primaryBlue, fontWeight: '800' }}>すべて見る</span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {recentInterests.map(student => (
                            <div 
                                key={student.id} 
                                style={{ ...styles.listItem, margin: 0, cursor: 'pointer' }}
                                onClick={() => onViewStudent(student.id)}
                            >
                                <div style={{ ...styles.avatar, background: `${primaryBlue}10`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: primaryBlue, fontWeight: '900' }}>
                                    {student.name[0]}
                                </div>
                                <div style={styles.listInfo}>
                                    <p style={styles.listTitle}>{student.name}</p>
                                    <p style={styles.listSubtitle}>{student.univ} ・ {student.time}</p>
                                </div>
                                <button 
                                    style={{ ...styles.button, backgroundColor: primaryBlue, color: '#fff', fontSize: '0.75rem', padding: '0.5rem 0.75rem' }}
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        onOpenChat({ id: 201, userId: student.id, name: student.name });
                                    }}
                                >
                                    スカウト
                                </button>
                            </div>
                        ))}
                    </div>
                </section>

                {/* Section: News/Feed */}
                <section>
                    <h3 style={{ fontSize: '1.1rem', fontWeight: '900', margin: 0, marginBottom: '1.25rem' }}>最近のキャンパスニュース</h3>
                    <div style={{ backgroundColor: '#FFFFFF', padding: '1.5rem', borderRadius: '24px', border: `1.5px dashed ${borderLight}`, textAlign: 'center' }}>
                        <p style={{ color: textSecondary, fontSize: '0.9rem', fontWeight: '700' }}>香川大学周辺で「IT系インターン」の検索が急増しています。</p>
                    </div>
                </section>
            </main>
        </div>
    );
};
